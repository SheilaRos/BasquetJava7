...
public class Consultas {
    ...
    public static List<Estadisticas> avgMaxMinJugadoresPosicion(List<Equipo> todosEquipos) {
        List<Estadisticas> avgPosicion = new ArrayList<>();
         Jugador jug = todosEquipos.get(0).getListaJugadores().getListaJugadores().get(0);
         Estadisticas nueva = new Estadisticas(jug.getPosicion(), jug.getCanastas(), jug.getAsistencias(), jug.getRebote());
         avgPosicion.add(nueva);
                for (Equipo e : todosEquipos) {
                    for (Jugador j : e.getListaJugadores().getListaJugadores()) {
                        Estadisticas estad = comprobar(avgPosicion, j.getPosicion());
                        if (estad != null) {
                            modificarEstadisticas(estad, j.getCanastas(), j.getAsistencias(), j.getRebote());
                        } else {
                            Estadisticas nueva1 = new Estadisticas(j.getPosicion(), j.getCanastas(), j.getAsistencias(), j.getRebote());
                            avgPosicion.add(nueva1);
                        }
                    }
                }
        return avgPosicion;
    }
    public static Estadisticas comprobar(List <Estadisticas> est, String posicion){
        Estadisticas estadistica = null;
        for(Estadisticas e : est){
            if(e.getPosicion().equalsIgnoreCase(posicion)){
                estadistica=e;
            }
        }
        return estadistica;
